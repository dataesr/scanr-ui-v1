...
const AapPage = (props) => {
  ...
  const getDataFromScanR = async () => {
    ...
    const should = [];
    keywords.forEach((kw) => {
      should.push({ match_phrase: { 'publications.publication.title.default': kw } });
      should.push({ match_phrase: { 'publications.publication.keywords.default': kw } });
      should.push({ match_phrase: { 'projects.project.label.en': kw } });
      should.push({ match_phrase: { 'projects.project.label.default': kw } });
      should.push({ match_phrase: { 'websites.webPages.content': kw } });
    });
      ...
      query: {
        bool: {
          ...
          filter: [
            { match: { status: 'active' } },
            { match: { isFrench: true } },
            { match: { 'kind.keyword': 'Secteur privé' } },
          ],
        },
      },
      highlight: {
    ...
    const headers = { headers: { Authorization: 'Basic c2NhbnI6c0M0blJfMm8ySSE=' } };

    Axios.post('https://scanr-api.enseignementsup-recherche.gouv.fr/elasticsearch/structures/_search', obj, headers).then((respFromScanR) => {
      // setResponsesFromScanR([respFromScanR]);
      console.log('respFromScanR', respFromScanR);
      // Etape suivante
    ...
    if (step.charAt(0) === '2') {
      return (
        <Col className={classes.steps}>
          <FormattedHTMLMessage id="step2" defaultMessage="step2" />
          <span className="mx-2">
